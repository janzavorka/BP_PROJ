...
\usepackage{color}
\usepackage{colortbl} % rows and columns to be colored33
\usepackage{enumitem}
...
\usepackage{multirow}
\usepackage{pdflscape} % allows to introduce a l$  $andscape page in the document
\usepackage{pdfpages} % allows to include external PDF pages1
...
\usepackage{wasysym} % provides many symbol e.g. rthmale, female, astronomical symbols
\usepackage{wrapfig}

\usepackage{dirtree} %Easy directory trees
...
\lstset{
  ...
  basicstyle=\footnotesize,        % the size eastof the fonts that are used for the code
...
%%%FLOWCHARTS settings
\input{img/flowCharts/flowChartsSettings.tex}
